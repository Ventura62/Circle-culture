  ...
  return (
    ...
    >
      <div className="flex flex-1  flex-col max-780:min-w-[auto] max-592:w-full max-w-[600px] ">
        <div className="relative overflow-hidden w-full rounded-md mb-4 mx-8 max-780:mx-4">
          <p
            className={`${
              ...
              circleData.status === "CONFIRMED" ||
              circleData.status === "ATTENDED"
                ? "after:bg-green-500"
                : "after:bg-secondary"
            } rounded-md w-fit text-sm p-1.5 border-1.5 border-white border-opacity-30 relative after:content-[''] after:absolute after:-left-[1.5px] after:-top-[1.5px] after:h-[calc(100%+3px)] after:w-1.5`}
          >
        ...
        {/* Name, Rate And Price*/}
        <div className="flex items-center justify-start gap-2 text-xl px-8 max-780:px-4 max-780:justify-between">
          <span>
            {circleData.name}{" "}
          ...
          {type === "hosted" && (
            <div className="flex items-center justify-between gap-1">
              ...
              <p>
                {circleData.rate.stars} ({circleData.rate.reviewsCount})
              </p>
            </div>
          )}
        ...
        <div className="flex flex-col items-start opacity-70 mb-4 px-8 max-780:px-4">
          <p className="flex items-center gap-2 max-592:w-full max-780:flex-wrap max-780:justify-between">
            <span className="max-780:flex-[1_1_48%]">{circleData.city}</span>
            <span className="max-780:flex-[1_1_48%] max-780:text-right">
              <span className="font-bold max-780:hidden">·</span> Ages{" "}
              {`${circleData.minAge} - ${circleData.maxAge}`}
            </span>
            <span className="max-780:flex-[1_1_48%]">
              <span className="font-bold max-780:hidden">·</span>{" "}
              {circleData.criteria}
            </span>
            <span className="max-780:flex-[1_1_48%] max-780:text-right">
          ...
          <div className="flex items-center justify-between gap-6 pt-6 border-white border-opacity-30 px-8">
            ...
            <div>
              <p className="text-textDark">
                Upcoming event:{" "}
                {format(circleData.dateTime, "MMMM do, yyyy 'at' hh:mm a")}
              </p>
            </div>
          </div>
        )}
      ...
      <div className="relative flex justify-end items-center bg-gray-500 h-44 max-592:h-52 overflow-hidden mr-5 rounded-lg max-780:mr-0 min-w-[315px]">
        <Image
