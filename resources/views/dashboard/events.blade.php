        ...
        <div class="container d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
            ...
            <div class="d-flex align-items-center mr-1">
                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    <!--begin::Page Title-->
                    <h2 style="color: #3a384e"  class="d-flex align-items-center font-weight-bolder my-1 mr-3">Events {{ Carbon\Carbon::now() }}</h2>
                    <!--end::Page Title-->
                </div>
                <!--end::Page Heading-->
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-head-custom table-vertical-center order-table kt_datatable" id="kt_advance_table_widget_1">
                            ...
                            <tr>
                                ...
                                <td >
                                    <span class="text-muted  font-weight-bold">{{ $row->id }}</span>
                                </td>

                                <td >
                                    <a href="{{ route('event.show' , $row['id']) }}" class="text-primary  font-weight-bold">{{ $row->name  }}  </a>
                                </td>

                                <td >
                                    <span class="text-muted  font-weight-bold">{{ Carbon\Carbon::parse($row->time)->format('F jS, Y h:i A') }}</span>
                                </td>
                                <td >
                                    <span class="text-muted  font-weight-bold">{{ $targetTime }}</span>
                                </td>

                                 <td>

                                     @if($currentTime->gt($targetTime))
                                         <span class="label label-lg label-warning label-inline mb-1">  {{ 'ended' }} </span>
                                     @else
                                         <span class="label label-lg @if($row->is_active) label-primary @else label-danger @endif label-inline mb-1" > @if($row->is_active) {{ 'active' }} @else {{ 'inactive' }} @endif </span>
                                     @endif
                                </td>

                                <td>
                                    @if(!$row->is_active)
                                        <a href="#" data-toggle="modal" data-target="#activate_record_modal" data-action="{{ route('event.activate' , $row->id) }}" data-id="{{ $row->id }}" class="open_activate_modal btn  btn-warning btn-hover-primary btn-sm">
                                           Activate
                                        </a >
                                    @endif
                                    ...
                                    <a href="#" data-toggle="modal" data-target="#delete_record_modal" data-action="{{ route('event.delete' , $row->id) }}" data-id="{{ $row->id }}" class="open_delete_modal btn btn-icon btn-light btn-hover-primary btn-sm">
                                        <span class="svg-icon svg-icon-md svg-icon-primary">
                    ...
                    <form class="form" method="POST" action="{{ route('event.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                            ...
                            {{--<div class="form-group row">--}}
                                ...
                                {{--<div class="col-lg-8">--}}
                                    {{--<input type="datetime-local" class="form-control" name="time" placeholder="Enter Name Here"  required />--}}
                                {{--</div>--}}
                            {{--</div>--}}
                                ...
                                <div class="col-lg-8">
                                    <select name="time" required class="form-control">
                                        ...
                                        @foreach($upcoming_dates as $date)
                                            <option  value="{{ $date['date'] }}">{{  Carbon\Carbon::parse($date['date'])->format('F jS, Y h:i A') }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <form class="form" method="POST" action="{{ route('event.store') }}" enctype="multipart/form-data">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group row">
                                    <label class="col-lg-4 col-form-label ">Name:</label>
                                    <div class="col-lg-8">
                                        <input type="text" class="form-control" name="name" placeholder="Enter Name Here" value="{{ $row['name'] }}" required />
                                    </div>
                                </div>
                                ...
                                <div class="form-group row ">
                                    ...
                                    <div class="col-lg-8">
                                        <select name="time" required class="form-control">
                                            ...
                                            @foreach($upcoming_dates as $date)
                                                <option @if($date['date'] ==  $row['time']) selected @endif value="{{ $date['date'] }}">{{  Carbon\Carbon::parse($date['date'])->format('F jS, Y h:i A') }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-8">
                                        <select name="is_active" required class="form-control">
                                            ...
                                            <option @if($row['is_active']) selected @endif value="1">Active</option>
                                            <option @if(!$row['is_active']) selected @endif value="0">Inactive</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
