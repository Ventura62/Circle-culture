    ...
    <div class="chat-container ">
        ...
        <div class="chat-area active">
            <div class="chat-header">
                <a  style="text-decoration: none; margin-right: 10px" href="{{ route('chat') }}" class="">←</a>
                <div class="avatar-container">
                    <img src="@if(isset($user['profile_pic'])) {{ asset($user['profile_pic']) }} @else {{ asset('avatar.png') }} @endif" alt="John Doe" class="avatar">
                    {{--<span class="status-dot ${user.status}"></span>--}}
                </div>
                <div class="user-info">
                    <h3>{{ $user->name }}</h3>
{{--                    <p>{{ $user->email }}</p>--}}
                    <input hidden class="receiver_id" value="{{ $receiver_id }}">
                </div>
                <div class="actions-dropdown" onclick="event.stopPropagation();">
                    <button class="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <div class="dropdown-menu">
                        <a href="{{ route('contact.remove', $user['id']) }}" class="dropdown-item submit">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
                            </svg>
                            Delete
                        ...
                        <a onclick="openModal(this)"  data-id="{{ $user['id'] }}" class="dropdown-item">
                            <span class="nav-link-name"><i class="fa fa-pen"></i></span>
                ...
                @foreach($messages as $message)
                    <div class="message @if($message->sender_id == auth()->user()->id) sent @else received @endif">
                        <div class="message-content">
                            <p>{{ $message->message }}</p>
                            {{--<div class="message-time">10-10-2021</div>--}}
                        </div>
                    </div>
                @endforeach
...
@section('scripts')
    ...
    <script>
        ...
        chat.bind('App\\Events\\MessageSent', function(data) {
            ...
            var content = '';
            if(data.message.receiver_id == "{!! auth()->user()->id !!}"){
